...
TRIBE v2 accepts video, audio, and/or text as input. `TribeModel.from_pretrained("facebook/tribev2")` returns a model that predicts a `Cortical_Response` of shape `[T, 20,484]` (fsaverage5 surface vertices). The paper describes a separately-trained subcortical variant producing `[T, 8,802]` over Harvard-Oxford atlas voxels, but that checkpoint is **not published** (HuggingFace `facebook/tribev2` ships only the cortical `best.ckpt` + `config.yaml`, hard-wired to TribeSurfaceProjector / fsaverage5). The cognitive similarity system therefore operates exclusively on the Cortical_Response. Cortical similarity measures how similarly two stimuli are *perceptually represented and understood* by the brain — capturing visual features, auditory features, semantic content, and language processing. Emotional similarity, memorability, and reward value — which are primarily subcortical — are out of scope both by design and because the subcortical weights are unavailable. The system must handle the temporal nature of the cortical output and support both whole-cortex and region-specific similarity queries.
...
- **TRIBE_v2**: The tri-modal brain encoding foundation model from Meta FAIR (HuggingFace: `facebook/tribev2`) that predicts whole-brain fMRI responses from video, audio, and/or text stimuli. Only the cortical variant is publicly released — see Brain_Response.
- **Brain_Response**: The predicted fMRI activation tensor produced by a single TRIBE_v2 model run on a Stimulus. In the public release this is exclusively a Cortical_Response of shape `[T, 20,484]` (see below); the paper describes a subcortical variant but its checkpoint is not published on HuggingFace (`facebook/tribev2` ships only `best.ckpt` + `config.yaml`, hard-wired to TribeSurfaceProjector on fsaverage5). Subcortical prediction is therefore out of scope for this system.
- **Cortical_Response**: A predicted fMRI activation tensor of shape `[T, 20,484]`, where T is the number of timepoints and 20,484 dimensions are cortical surface vertices in fsaverage5 space. This is the sole input to all similarity computations.
...
- **ICA_Network_Mask**: A continuous vector of 20,484 values derived by running FastICA on TRIBE v2's "unseen subject" projection layer — a matrix of shape `(2048, 20,484)` embedded in the model weights (`best.ckpt`). The 2,048 dimension comes from the `low_rank_head` bottleneck in the model architecture (confirmed in `config.yaml`). Each of the 5 ICA components is a vector over all cortical vertices. **Label assignment**: per TRIBEv2.pdf §5.10 *"the order of the components does not carry any meaning for ICA"*, so we do not treat FastICA's output order as meaningful. Instead, labels are assigned by spatial Pearson correlation against five NeuroSynth keyword maps ("primary auditory", "language", "motion", "default network", "visual") projected to fsaverage5, with a bipartite 1-to-1 assignment (Hungarian algorithm on |r|) and sign-flipping for negatively matched components. Assignment + correlations are persisted in `ica_masks.npz` with full provenance (see Requirement 3). The system supports two modes for using these components: (1) **binary mask mode** — threshold at the top 10% of absolute values (~2,048 vertices), consistent with the paper's Figure 6A visualization; (2) **continuous weighting mode** — standard weighted Pearson over all 20,484 vertices with `|component|` as weights. Binary mask mode is the default.
...
2. THE System SHALL store the Cortical_Response in cache (as `raw_cortical.npy`) keyed by the stimulus's content hash, so that subsequent queries can reuse it without re-running inference. No subcortical tensor is obtained or stored: Meta publishes only the cortical `best.ckpt` on HuggingFace; a `MaskProjector(mask="subcortical")` override is documented in the library's training grid but the corresponding trained weights are not released, so the "subcortical" predict() call would silently return cortical-shaped output.
...
1. THE System SHALL compute the five ICA_Network_Masks at initialization by loading `best.ckpt` from HuggingFace (`facebook/tribev2`), extracting the unseen-subject projection layer (shape `2048 × 20,484`, where 2,048 is the `low_rank_head` bottleneck), running `FastICA(n_components=5)` with sufficient `max_iter` to converge (empirically ~3998 iterations; `FASTICA_MAX_ITER=10_000` with a RuntimeError if the cap is hit), and thresholding each component at the top 10% of absolute values. Computed masks SHALL be cached locally to avoid recomputation on subsequent runs. **Cache schema SHALL include**: the 5 components, their 5 binary masks, the label-to-component assignment (dict), the |Pearson r| with each network's NeuroSynth reference map, the sklearn version that produced the components, and the FastICA iteration count at convergence.
1a. THE System SHALL assign ICA_Network labels to components via the §5.10 procedure: spatial Pearson correlation between each component and five NeuroSynth keyword maps ("primary auditory", "language", "motion", "default network", "visual"), followed by a bipartite 1-to-1 assignment that maximizes total |r| (Hungarian algorithm via `scipy.optimize.linear_sum_assignment`). Negatively-correlated matches SHALL flip the component's sign so the cached components are all positively oriented with respect to their NeuroSynth references. When the NeuroSynth term maps file is absent from the cache directory, the system SHALL log a loud WARNING pointing to `scripts/fetch_neurosynth_maps.py` and fall back to positional labels (legacy behavior — documented as not paper-faithful).
2. THE System SHALL support two ICA network similarity modes:
   ...
   - **Continuous weighting mode**: standard weighted Pearson correlation over all 20,484 vertices with weights `|component|` (weighted mean centering + weighted covariance). With uniform weights this collapses exactly to standard Pearson.
...
3. THE System SHALL use Pearson correlation (not cosine similarity) as the similarity metric, as it is mean-centered and robust to differences in overall activation magnitude between stimuli. **Framing note** (Slice 5 B1): we reuse the spatial Pearson correlation metric that the TRIBE v2 paper applies in sections 2.5, 2.6, and 5.10, but the paper applies it between *predicted and measured contrast maps* (or between predicted maps and NeuroSynth reference maps), not between the predicted responses of two individual stimuli. Our pairwise-between-stimuli construction is a novel application of the metric that the paper does not independently validate; it rests on the assumption that two category-matched stimuli will activate the same ICA-masked vertices more similarly than a cross-category pair. See `cognitive_similarity/paper_replication.py` for the paper-faithful §5.9 contrast-map methodology used as a separate sanity check.
...
3. THE System SHALL validate the following expected orderings, each grounded in a specific finding from the TRIBE v2 paper:
   ...
   **Language Network ICA network** (section 2.6 — RSVP tasks):
...
6. WHEN stimuli for any of the 6 orderings are unavailable in the cache, THE System SHALL mark the corresponding check as skipped (not failed) with a descriptive "MISSING" log entry so that partial-coverage releases remain interpretable.

#### Orderings dropped in Slice 3 (pruned from the original 9-check set)

- `sim(audio_segment, audio_segment) > sim(audio_segment, silence)` — **dropped**. Santoro 2017 clean-sound vs synthesized-silence is not a §5.9 construction; the paper's speech-vs-silence contrast uses Algonauts movie audio. Pairwise Pearson at t=5 on a silent stimulus reflects TRIBE's baseline prediction, not anything informative about auditory cortex.
- `sim(complex_sentence, complex_sentence) > sim(complex_sentence, simple_sentence)` — **dropped**. TRIBEv2.pdf §2.6's complex/simple finding is a *magnitude* contrast in Broca's area; translating that to a pairwise-similarity ordering requires assumptions the paper does not make (specifically: that two complex sentences about different topics share more Broca variance than a complex/simple pair). The metric mismatch is the issue, not the stimulus.
- `sim(motion_video, motion_video) > sim(motion_video, static_image)` — **dropped**. The paper has no 1-s in-silico protocol for motion, so there's no direct paper grounding for this pairwise construction. The MT+ cognitive axis remains a product feature (component 0 after §5.10 assignment) and is validated indirectly by the NeuroSynth spatial correlation (|r|≈0.47) in `ica_masks.npz`.

#### Validation status (as of Slice 5, pre-Modal-re-inference cache)

6-check validation with NeuroSynth §5.10 labels + Benjamini-Hochberg FDR across the batch. 5/6 orderings hold with stable Δ > 0 and positive CI; 0/6 show mask-specific selectivity at BH q<0.05. Details in `CLAUDE.md` §Validation Status. The current cache uses pre-Slice-3 10 s stimulus tensors; re-running Modal with the §5.9-aligned 1 s + 7 s blank preprocessor may shift these numbers.
